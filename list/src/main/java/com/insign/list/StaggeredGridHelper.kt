...
class StaggeredGridHelper<T>: ListingHelper<T> {
    ...
    override fun make(adapter: RecyclerView.Adapter<RecyclerView.ViewHolder>) {
        if (list is RecyclerView) {
            list.layoutManager = StaggeredGridLayoutManager(numOfColumn, orientation.value())
    ...
    class Builder<T>: ListingHelper.Builder<T> {
        ...
        override fun build() = StaggeredGridHelper(this)
    }
}